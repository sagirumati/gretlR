...
import_kable1=function(path=".",chunk,file,start=NA,end=NA,skip_blank=FALSE,format=kable_format(), digits = getOption("digits"), row.names = NA,
...
path=path
  ...
  # path=read.csv(path,allowEscapes = T,header = T,check.names = FALSE)

  path=readxl::read_xlsx(path)


  if(!is.na(start) && is.na(end)){
    # path=readLines(path)
    path=path[start:nrow(path),]
...
if(skip_blank==T) path=path[!apply(path=="",1, all),]
# path=write.csv(path,"gretlr.csv",sep = ",")
return(knitr::kable(path, format = format, digits = digits,row.names = row.names, col.names = col.names, align = align, caption = caption, label = label, format.args = format.args, escape = escape, ...))
